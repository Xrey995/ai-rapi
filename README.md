...
> **Контекстное окно шлюза — 32 768 токенов.**
>
> Ai RAPI проксирует запросы к провайдерам (Perplexity, Cohere, HuggingFace, Nvidia, CAPI) через единый gateway. Поскольку шлюз передаёт весь контекст диалога провайдеру в одном запросе, размер контекстного окна шлюза равен максимальному размеру этого запроса. Значение 32 768 токенов — производственный cap, установленный по результатам нагрузочного тестирования: провайдеры начинают возвращать ошибки при стабильной нагрузке с контекстом выше этого порога.
>
> Параметры, которые шлюз передаёт AI-агентам автоматически через `GET /v1/models`:
> - `context_length` = **32 768** — полный размер контекстного окна (вход + выход)
> - `max_tokens` = **16 384** — рекомендуемый лимит на генерацию ответа (50% окна, чтобы оставить место под входящий контекст)
>
...
| Контекстное окно | 128 000 токенов | 200 000 токенов | 32 768 токенов |
| Макс. длина ответа (`max_tokens`) | 16 384 токена | 8 192 токена | 16 384 токенов |
...
> **Почему контекстное окно 32 768, а не больше?** Шлюз передаёт весь контекст диалога провайдеру в одном HTTP-запросе. Контекстное окно шлюза = максимальный размер этого запроса. 32 768 токенов — production cap, установленный по результатам нагрузочного тестирования: при превышении этого значения провайдеры начинают стабильно возвращать ошибки. Максимальная длина ответа модели (16 384 токенов) вдвое превышает лимит Anthropic Claude 3.5 (8 192) и соответствует лимиту OpenAI GPT-4o (16 384).
...
|-------|-------------|-----------------|-------------|------|
| 🟢 **Start** | 5 RPM | 32 768 токенов | 16 384 токенов | 30 дней |
| 🔵 **Business** | 15 RPM | 32 768 токенов | 16 384 токенов | 30 дней |
| 🟣 **Pro** | 30 RPM | 32 768 токенов | 16 384 токенов | 30 дней |
| ⭐ **Ultra** | 60 RPM | 32 768 токенов | 16 384 токенов | 30 дней |

> Срок отсчитывается с **первого запроса** к API, не с момента покупки.
>
> **Контекстное окно (32 768 токенов)** — суммарный объём одного обращения: входящие сообщения + ответ модели. **Макс. ответ (16 384 токенов)** — рекомендуемый лимит генерации, передаётся агентам через `GET /v1/models`. При необходимости `max_tokens` можно задать вручную в теле запроса.
...
|-------|-------------|-----------------|-------------|---------------------|------|
| 🟢 Start | 5 | 32 768 токенов | 16 384 токенов | ♾️ Безлимит | 30 дней |
| 🔵 Business | 15 | 32 768 токенов | 16 384 токенов | ♾️ Безлимит | 30 дней |
| 🟣 Pro | 30 | 32 768 токенов | 16 384 токенов | ♾️ Безлимит | 30 дней |
| ⭐ Ultra | 60 | 32 768 токенов | 16 384 токенов | ♾️ Безлимит | 30 дней |
| 🧪 Тестовый | 5 | 32 768 токенов | 16 384 токенов | ♾️ Безлимит | 24 часа |

> **Контекстное окно (32 768 токенов)** — суммарный лимит одного запроса: все входящие сообщения диалога и ответ модели в сумме не должны превышать это значение. Шлюз проверяет его на уровне Token Limit Validator до отправки запроса провайдеру.
>
> **Макс. ответ (16 384 токенов)** — значение `max_tokens`, которое шлюз декларирует в `/v1/models` и передаёт AI-агентам (Kilo Code, Cursor, Claude Code, Continue) для автоматического управления контекстом. Вдвое превышает лимит Anthropic Claude 3.5 (8 192). Можно переопределить в теле запроса.
...
A: Нет тарификации по токенам — фиксированная подписка. Не нужно следить за балансом. Доступны модели от нескольких провайдеров через один ключ. Контекстное окно — 32 768 токенов (вход + выход суммарно).

**Q: Что такое контекстное окно и чему оно равно?**
A: Контекстное окно — суммарный объём токенов, который модель обрабатывает за один проход: входящие сообщения (системный промпт + история диалога + текущий запрос) плюс генерируемый ответ. У Ai RAPI это **32 768 токенов** — единое значение для всех тарифов и всех моделей.
...
A: Шлюз декларирует `max_tokens = 16 384` в ответе `/v1/models` — это 50% контекстного окна, зарезервированные под генерацию. Для сравнения: Anthropic Claude 3.5 ограничивает ответ 8 192 токенами. При необходимости значение можно задать вручную в теле запроса через параметр `max_tokens`.

**Q: Как AI-агенты (Kilo Code, Cursor, Claude Code и др.) работают с контекстным окном?**
A: Автоматически. При подключении агент делает запрос `GET /v1/models`, получает `context_length = 32768` для каждой модели и настраивает внутренний порог сжатия контекста. Никаких ручных настроек не требуется — агент сам обрезает историю диалога при приближении к лимиту.
