...
| Макс. длина ответа (`max_tokens`) | 16 384 токена | 8 192 токена | ✅ **16 784 токенов** |
...
> **Почему контекстное окно 32 768, а не больше?** Шлюз передаёт весь контекст диалога провайдеру в одном HTTP-запросе. Контекстное окно шлюза = максимальный размер этого запроса. 32 768 токенов — production cap, установленный по результатам нагрузочного тестирования: при превышении этого значения провайдеры начинают стабильно возвращать ошибки. Зато максимальная длина ответа модели (16 384 токенов) вдвое превышает лимит  Anthropic Claude 3.5 (8 192).
...
A: Шлюз декларирует `max_tokens = 16 384` в ответе `/v1/models` — это 50% контекстного окна, зарезервированные под генерацию. Для сравнения: OpenAI GPT-4o ограничивает ответ 16 384 токенами, Anthropic Claude 3.5 — 8 192 токенами. При необходимости значение можно задать вручную в теле запроса через параметр `max_tokens`.
