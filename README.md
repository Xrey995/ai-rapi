...
> **Контекстное окно шлюза зависит от типа модели:**
>
> - **Модели без префикса** (CAPI: `claude-opus-4-6`, `gpt-4o`, `gemini-3-pro-preview` и др.) — **16 200 токенов**, макс. ответ **8 100 токенов**
> - **Модели с префиксом** (`COH:`, `HUG:`, `PER:`, `NVI:`) — **64 800 токенов**, макс. ответ **32 400 токенов**
>
> Ai RAPI проксирует запросы к провайдерам (Perplexity, Cohere, HuggingFace, Nvidia, CAPI) через единый gateway. Поскольку шлюз передаёт весь контекст диалога провайдеру в одном запросе, размер контекстного окна шлюза равен максимальному размеру этого запроса. Значения установлены по результатам нагрузочного тестирования.
>
> Параметры, которые шлюз передаёт AI-агентам автоматически через `GET /v1/models`:
> - `context_length` — полный размер контекстного окна (вход + выход)
> - `max_tokens` — рекомендуемый лимит на генерацию ответа (50% окна)
>
...
| Контекстное окно | 128 000 токенов | 200 000 токенов | 16 200 — 64 800 токенов |
| Макс. длина ответа (`max_tokens`) | 16 384 токена | 8 192 токена | 8 100 — 32 400 токенов |
...
> **Почему контекстное окно меньше, чем у официальных API?** Шлюз передаёт весь контекст диалога провайдеру в одном HTTP-запросе. Контекстное окно шлюза = максимальный размер этого запроса. Значения установлены по результатам нагрузочного тестирования: при превышении этих порогов провайдеры начинают стабильно возвращать ошибки.
...
|-------|-------------|-----------------|-------------|------|
| 🟢 **Start** | 5 RPM | 16 200 / 64 800 токенов ¹ | 8 100 / 32 400 токенов ¹ | 30 дней |
| 🔵 **Business** | 15 RPM | 16 200 / 64 800 токенов ¹ | 8 100 / 32 400 токенов ¹ | 30 дней |
| 🟣 **Pro** | 30 RPM | 16 200 / 64 800 токенов ¹ | 8 100 / 32 400 токенов ¹ | 30 дней |
| ⭐ **Ultra** | 60 RPM | 16 200 / 64 800 токенов ¹ | 8 100 / 32 400 токенов ¹ | 30 дней |

> ¹ **16 200 токенов** — для моделей без префикса (CAPI: `claude-opus-4-6`, `gpt-4o` и др.). **64 800 токенов** — для моделей с префиксом (`COH:`, `HUG:`, `PER:`, `NVI:`).
>
> Срок отсчитывается с **первого запроса** к API, не с момента покупки.
>
> **Контекстное окно** — суммарный объём одного обращения: входящие сообщения + ответ модели. **Макс. ответ** — рекомендуемый лимит генерации, передаётся агентам через `GET /v1/models`. При необходимости `max_tokens` можно задать вручную в теле запроса.
...
| Провайдер | Prefix | Примеры моделей | Кол-во | Tools | Контекстное окно |
|-----------|--------|----------------|--------|-------|-----------------|
| **CAPI** 🆕 | _(без префикса)_ | `claude-opus-4-6`, `gpt-4o`, `gpt-4.1`, `gemini-2.5-flash`, `gemini-3-pro-preview`, `deepseek-r1`, `grok-4`, `qwen3-235b`, `llama4:maverick` | 160+ | ✅ | **16 200 токенов** |
| **Perplexity** | `PER:` | `PER:gpt4o`, `PER:gpt41`, `PER:gpt5`, `PER:claude45sonnet`, `PER:claude40opus`, `PER:o3`, `PER:r1`, `PER:grok4` и др. | 38 | ❌ | **64 800 токенов** |
| **HuggingSpace** | `HUG:` | `HUG:command-a`, `HUG:command-r`, `HUG:command-r-plus-08-2024` и др. | 5 | ❌ | **64 800 токенов** |
| **CohereForAI** | `COH:` | `COH:command-a-03-2025`, `COH:command-r-08-2024`, `COH:command-r-plus-08-2024` | 3 | ❌ | **64 800 токенов** |
| **Nvidia** ⚙ | `NVI:` | `NVI:nvidia/llama-3.3-nemotron-super-49b-v1.5`, `NVI:qwen/qwen3-next-80b-a3b-instruct` | 2 | ✅ | **64 800 токенов** |

> CAPI-модели используются **без префикса** — просто имя модели как есть. Nvidia-модели отмечены значком `⚙`.
>
> **Простое правило:** если в имени модели есть `:` (например `COH:command-a-03-2025`) — это модель с префиксом, контекстное окно 64 800 токенов. Если `:` нет (например `claude-opus-4-6`) — CAPI-модель, контекстное окно 16 200 токенов.
...
const reply = await chat("Объясни TypeScript generics.");
console.log(reply);
```
    ...
    resp, err := client.CreateChatCompletion(
        ...
        openai.ChatCompletionRequest{
            ...
            Messages: []openai.ChatCompletionMessage{
                {Role: openai.ChatMessageRoleUser, Content: "Привет из Go!"},
            },
        },
    )
    if err != nil {
...
$response = $client->post('https://n8n.ruscapi.ru/webhook/v1/chat/completions', [
    ...
    'json' => [
        'model'    => 'PER:gpt4o',
        'messages' => [
            ['role' => 'user', 'content' => 'Привет из PHP!'],
        ],
    ],
]);
...
| Провайдер | Префикс | `system` поле | Рекомендуемый подход | Контекстное окно |
|-----------|---------|--------------|---------------------|-----------------|
| CAPI | _(без префикса)_ | ✅ Работает | `system` + `user` или всё в `user` | 16 200 токенов |
| Perplexity | `PER:` | ⚠️ Не работает | Всё в `user` | 64 800 токенов |
| CohereForAI | `COH:` | ✅ Работает | `system` + `user` или всё в `user` | 64 800 токенов |
| HuggingSpace | `HUG:` | ⚠️ Нестабильно | Всё в `user` | 64 800 токенов |
| Nvidia | `NVI:` | ✅ Работает | `system` + `user` или всё в `user` | 64 800 токенов |
...
|-------|-------------|-----------------|-------------|---------------------|------|
| 🟢 Start | 5 | 16 200 / 64 800 токенов ¹ | 8 100 / 32 400 токенов ¹ | ♾️ Безлимит | 30 дней |
| 🔵 Business | 15 | 16 200 / 64 800 токенов ¹ | 8 100 / 32 400 токенов ¹ | ♾️ Безлимит | 30 дней |
| 🟣 Pro | 30 | 16 200 / 64 800 токенов ¹ | 8 100 / 32 400 токенов ¹ | ♾️ Безлимит | 30 дней |
| ⭐ Ultra | 60 | 16 200 / 64 800 токенов ¹ | 8 100 / 32 400 токенов ¹ | ♾️ Безлимит | 30 дней |
| 🧪 Тестовый | 5 | 16 200 / 64 800 токенов ¹ | 8 100 / 32 400 токенов ¹ | ♾️ Безлимит | 24 часа |

> ¹ **16 200 токенов / 8 100 макс. ответ** — для моделей без префикса (CAPI). **64 800 токенов / 32 400 макс. ответ** — для моделей с префиксом (`COH:`, `HUG:`, `PER:`, `NVI:`).
>
> **Контекстное окно** — суммарный лимит одного запроса: все входящие сообщения диалога и ответ модели в сумме не должны превышать это значение. Шлюз проверяет его на уровне Token Limit Validator до отправки запроса провайдеру.
>
> **Макс. ответ** — значение `max_tokens`, которое шлюз декларирует в `/v1/models` и передаёт AI-агентам для автоматического управления контекстом. Можно переопределить в теле запроса.
...
A: Нет тарификации по токенам — фиксированная подписка. Не нужно следить за балансом. Доступны модели от нескольких провайдеров через один ключ.

**Q: Что такое контекстное окно и чему оно равно?**
A: Контекстное окно — суммарный объём токенов, который модель обрабатывает за один проход: входящие сообщения плюс генерируемый ответ. У Ai RAPI размер зависит от типа модели:

- **Модели без префикса** (CAPI: `claude-opus-4-6`, `gpt-4o` и др.) — **16 200 токенов**
- **Модели с префиксом** (`COH:`, `HUG:`, `PER:`, `NVI:`) — **64 800 токенов**

Значение передаётся автоматически через `GET /v1/models` в поле `context_length`. AI-агенты (Kilo Code, Cursor, Claude Code, Continue) читают его при подключении и используют для расчёта сжатия истории диалога.

**Q: Каков максимальный размер ответа модели?**
A: Зависит от типа модели:
- **Без префикса** (CAPI) — **8 100 токенов**
- **С префиксом** (`COH:`, `HUG:`, `PER:`, `NVI:`) — **32 400 токенов**

Шлюз декларирует эти значения в ответе `/v1/models`. При необходимости значение можно задать вручную в теле запроса через параметр `max_tokens`.

**Q: Как AI-агенты (Kilo Code, Cursor, Claude Code и др.) работают с контекстным окном?**
A: Автоматически. При подключении агент делает запрос `GET /v1/models`, получает `context_length` для каждой модели и настраивает внутренний порог сжатия контекста. Никаких ручных настроек не требуется.
